...
export async function migrateSqliteToMysql(cfg: {
  ...
  try {
    ...
    for (const ex of allExecutions) {
      await connection.query(
        'INSERT INTO executions (id, equipment_id, year, month, scheduled_date, is_executed, executed_date, responsible_name, responsible_role, signature_data_url, completed_tasks, parts_replaced, observations, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [ex.id, ex.equipmentId, ex.year, ex.month, ex.scheduledDate, ex.isExecuted ? 1 : 0, ex.executedDate, ex.responsibleName, ex.responsibleRole, ex.signatureDataUrl, ex.completedTasks, ex.partsReplaced, ex.observations, ex.createdAt, ex.updatedAt]
      );
    }
