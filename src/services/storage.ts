...
export const StorageService = {
  ...
  // Guarda o actualiza un registro individual de ejecución (preventivo o correctivo)
  async upsertExecution(execution: MaintenanceExecution): Promise<void> {
    ...
    const index = list.findIndex((e) => {
      if (execution.id && e.id === execution.id) return true;
      if (execution.maintenanceType === 'correctivo') return false;
      return e.equipmentId === execution.equipmentId && e.year === execution.year && e.month === execution.month && e.maintenanceType !== 'correctivo';
    });
    if (index >= 0) {
      ...
      const newExecution: MaintenanceExecution = {
        ...execution,
        id: execution.id || `${execution.equipmentId}_${execution.year}_${execution.month}_${Date.now()}`,
        createdAt: execution.createdAt || new Date().toISOString(),
        updatedAt: new Date().toISOString()
      };
      list.push(newExecution);
    }
    await this.saveExecutions(list);
  },

  // Registra un mantenimiento correctivo nuevo
  async recordCorrectiveMaintenance(execution: Partial<MaintenanceExecution> & { equipmentId: string }): Promise<MaintenanceExecution> {
    const now = new Date();
    const fullExecution: MaintenanceExecution = {
      id: execution.id || `corr_${execution.equipmentId}_${Date.now()}`,
      equipmentId: execution.equipmentId,
      year: execution.year || now.getFullYear(),
      month: execution.month || (now.getMonth() + 1),
      scheduledDate: execution.scheduledDate || now.toISOString().split('T')[0],
      isExecuted: true,
      executedDate: execution.executedDate || now.toISOString().slice(0, 16).replace('T', ' '),
      responsibleName: execution.responsibleName || 'Técnico de Mantenimiento FCBV',
      responsibleRole: execution.responsibleRole || 'Técnico FCBV',
      signatureDataUrl: execution.signatureDataUrl,
      completedTasks: execution.completedTasks || [],
      partsReplaced: execution.partsReplaced || [],
      observations: execution.observations || '',
      maintenanceType: 'correctivo',
      failureDescription: execution.failureDescription || '',
      actionTaken: execution.actionTaken || '',
      downtimeHours: execution.downtimeHours || 0,
      finalOperationalStatus: execution.finalOperationalStatus || 'operativo',
      createdAt: now.toISOString(),
      updatedAt: now.toISOString()
    };

    await this.upsertExecution(fullExecution);
    return fullExecution;
  },

  // Busca en la base de datos de equipos por código exacto, número de serie o ID
  findEquipmentByCodeOrSerial(query: string, equipments: Equipment[]): Equipment | undefined {
    if (!query) return undefined;
    const clean = query.trim().toUpperCase();
    return equipments.find(
      (e) =>
        e.code.toUpperCase() === clean ||
        (e.serial && e.serial.toUpperCase() === clean) ||
        e.id === query.trim() ||
        clean.includes(e.code.toUpperCase()) ||
        (e.serial && clean.includes(e.serial.toUpperCase()))
    );
  },
